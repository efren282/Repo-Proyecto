...
export default class Muestra_Producto extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    ...
    const url =
      'https://mppag.000webhostapp.com/Seminario_Bases_Datos/Empleado/muestra_producto.php?id='+this.state.id;
    fetch(url)
      .then((response) => response.json())
      .then((responseJson) => {
        this.setState({
          isLoading: false,
          dataSource: responseJson.producto
        });
      })
      .catch((error) => {
        console.log(error);
